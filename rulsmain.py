...
class RULDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(0,4):
            if self.y[index]-i>=0:
                gt.append(self.y[index][0]-i)
            else:
                gt.append(0)
        # print(gt)
        return history,np.array(gt)
    def __len__(self):
        return self.len
def main():  
    ...
    for datasetname in datasetnames:
        print(f"now is running {datasetname}")
        sequence_length=32
        alpha = 0.1
        ...
        # model=models.BiMamba4RULs.Model(num_scales,seq_len,ch_ind,stride,patch_len,d_model,d_state,d_conv,e_fact,d_ff,dropout,bi_dir,residual,e_layers,enc_in,sp_num,sp_d_model)
        ...
        with torch.no_grad():
            for i, (history, rul) in enumerate(test_loader):
                ...
                rmse_loss = torch.sqrt(F.mse_loss(output[0], rul[0].view_as(output), reduction='mean'))
                if i == 203 and datasetname == 'FD004':
                    temp=rulutils.score([rul.cpu().detach().numpy()[0][0]], [output.cpu().detach().numpy()[0][0]])
                    print("have odd data!")
                    print(f"no.{i} socre : {temp}")
                    test_score += temp
                else:
                    # print([rul.cpu().detach().numpy()[0][0]])
                    temp=rulutils.score(np.array([rul.cpu().detach().numpy()[0][0]]), np.array([output.cpu().detach().numpy()[0][0]]))
                    print(f"no.{i} socre : {temp}")
                    test_score += temp
                                # print(f"no.{i} socre: {temp}")
