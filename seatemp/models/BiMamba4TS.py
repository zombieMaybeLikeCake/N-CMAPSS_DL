...
class Model(nn.Module):
    def __init__(self, configs, corr=None):
        ...
        self.revin = configs.revin  #revin=1
        self.embed_type = configs.embed_type #embed_type=0
        if configs.SRA: #SRA=False
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if not self.ch_ind:
            ...
            # enc_out size torch.Size([224, 23, 64]
        ...
        # dec_out size torch.Size([32, 7, 23, 64])
        dec_out = self.head(dec_out).permute(0, 2, 1)[:, :, :M]
        # dec_out size torch.Size([32, 96, 7])
        if self.revin:
