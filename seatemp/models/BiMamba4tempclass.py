...
class Model(nn.Module):
    ...
    def __init__(self, num_scales, seq_len, ch_ind, stride, patch_len, d_model, d_state, d_conv,
                 e_fact, d_ff, dropout, bi_dir, residual, e_layers, enc_in, sp_num, sp_d_model, num_classes):
        ...
        self.num_classes = num_classes  # **新增這個參數**

        self.local_token_layer = PatchEmbedding(self.seq_len, self.d_model, self.patch_len, self.stride, self.dropout, 
                                                nn.Identity(), pos_embed_type='sincos', learnable=True, ch_ind=self.ch_ind)

        self.space_token_layer = PatchEmbedding(self.d_model, self.sp_d_model, sp_num, sp_num, self.dropout,
                                                nn.Identity(), pos_embed_type='sincos', learnable=True, ch_ind=self.ch_ind)

        # **確保輸出為 num_classes**
        self.final_linear = nn.Linear(self.num_scales * self.num_classes, self.num_classes)
        ...
        for i in range(self.num_scales):
            ...
            self.BiMambaEncoders.append(TwoStageBiMambaEncoder(current_d_model, d_ff, self.space_token_layer, sp_d_model))
            self.dropouts.append(nn.Dropout(self.dropout))
            self.linears.append(nn.Linear(current_d_model * self.sp_d_model, self.num_classes))
    ...
    def loss_function(self,target,prediction):
        ...
        if(prediction_size == torch.Size([])):
            prediction=torch.tensor([prediction.item()], device='cuda:0', requires_grad=True)
            target=target[0]
        # else:
            # print("use")
