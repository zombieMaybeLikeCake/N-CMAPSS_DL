...
class Model(nn.Module):
    def __init__(self,num_scales,seq_len,ch_ind,stride,patch_len,d_model,d_state,d_conv,e_fact,d_ff,dropout,bi_dir,residual,e_layers,enc_in,sp_num,sp_d_model):
        ...
        self.final_linear=nn.Linear(self.num_scales*4,4)
        ...
        for i in range(self.num_scales):
            ...
            self.BiMambaEncoders.append(TwoStageBiMambaEncoder(current_d_model,self.d_ff,self.space_token_layer,sp_d_model))
            self.dropouts.append(nn.Dropout(self.dropout))
            self.linears.append(nn.Linear(current_d_model*self.sp_d_model,4))
    ...
    def loss_function(self,target,prediction):
        ...
        reg_loss = torch.sqrt(F.mse_loss(prediction, target, reduction='mean'))
