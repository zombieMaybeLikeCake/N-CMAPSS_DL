...
def get_data(dataset, sensors, sequence_length, alpha, threshold):
	...
	test_gen = (list(gen_test_data(X_test_pre[X_test_pre['unit_nr']==unit_nr], sequence_length, sensors, 0.,unit_nr))
			   for unit_nr in X_test_pre['unit_nr'].unique())
	x_test = np.concatenate(list(test_gen)).astype(np.float32)
	
	return x_train, y_train, x_val, y_val, x_test, y_test['RemainingUsefulLife'].values.reshape(-1, 1)
# ---------------------------------------------------------------------------------------------------
def get_certain_test_data(dataset, sensors, sequence_length, alpha, threshold,specifyno):
	# files
	dir_path = './data/'
	train_file = 'train_'+dataset+'.txt'
	test_file = 'test_'+dataset+'.txt'
    ...
    # create RUL values according to the piece-wise target function 分段目標函數
	train = add_remaining_useful_life(train)
    ...
    # exponential_smoothing(df, sensors, n_samples, alpha=0.4)
	X_train_pre= exponential_smoothing(X_train_pre, sensors, 0, alpha)
	X_test_pre = exponential_smoothing(X_test_pre, sensors, 0, alpha)
	X_train_pre = X_train_pre[X_train_pre['unit_nr'] == specifyno]
	X_test_pre = X_test_pre[X_test_pre['unit_nr'] == specifyno]
	train_unit = X_train_pre['unit_nr'].unique()
	test_unit = X_test_pre['unit_nr'].unique()
	last_rul=y_test['RemainingUsefulLife'].iloc[specifyno-1]
	x_train = gen_data_wrapper(X_train_pre, sequence_length, sensors, train_unit)
	y_train = gen_label_wrapper(X_train_pre, sequence_length, ['RUL'], train_unit)
	# create sequences for test 
	x_test = gen_data_wrapper(X_test_pre, sequence_length, sensors, test_unit)
	y_test = gen_label_wrapper(X_test_pre, sequence_length, ['RUL'], test_unit)
	y_test+=last_rul
...
#   print("score: ", res)
  return res
def score_in_train(y_true, y_hat):
  res = 0
  for true, hat in zip(y_true, y_hat):
    subs = hat - true
    if subs < 0:
